...
<body>
    ...
    <div class="py-5">
        <h1 class="display-4 fw-bolder">Añadir Chelines</h1>
        <div class="main-agileinfo">
            <div class="agileits-top">
                <form action=<?php echo "../Controlers/controlUpdate.php?data1=addChelines&data2=".$idUser?> method="POST">
                <select id="payMethod" name="payMethod" class="contenedor"  required="">
                   <?php
                   foreach ($payMethods as $pm) {
                    echo "<option value=************".$pm->getIdPayMethod().">************".substr($pm->getCardNumber(),-4)."</option>";
                }
                   ?>
                    <input id="amount" name="amount" placeholder="Monto a Recargar" type="number" class="btn btn-outline-dark mt-auto" required="">
                    
                    <input id="button" class="btn btn-outline-dark " type="submit" value="Recargar">
                </form>
            </div>
        </div>
    </div>
